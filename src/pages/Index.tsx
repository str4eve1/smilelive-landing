      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <aside className="relative overflow-hidden rounded-[22px] border border-orange-300/40 bg-[linear-gradient(150deg,rgba(249,115,22,0.16),rgba(18,28,43,0.6))] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.14),0_30px_78px_-58px_rgba(0,0,0,0.88)] backdrop-blur">
            <strong className="block text-3xl font-body font-black tracking-tight leading-none text-white">Tutto incluso.</strong>
  ...
  return (
    <section id="pricing" className="py-16 md:py-28 bg-white relative overflow-hidden" style={{ backgroundImage: 'radial-gradient(50% 55% at 12% 18%, rgba(56,189,248,0.20), transparent 70%), radial-gradient(48% 55% at 88% 22%, rgba(129,140,248,0.20), transparent 70%), radial-gradient(60% 60% at 50% 108%, rgba(59,130,246,0.14), transparent 68%)', scrollMarginTop: 'var(--header-h, 7rem)' }}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          {/* FREE */}
          <motion.div variants={fadeUp} className="bg-white rounded-3xl border border-slate-200 shadow-[0_10px_35px_rgba(15,23,42,0.06)] p-8 flex flex-col items-center text-center hover:shadow-[0_16px_50px_rgba(2,132,199,0.12)] hover:border-primary/30 hover:-translate-y-1 transition-all duration-300">
          ...
          {studioPlans.map((plan) => {
            ...
            return (
              <motion.div
                key={plan.name}
                variants={fadeUp}
                className={`rounded-3xl p-8 flex flex-col items-center text-center relative ${plan.highlighted
                  ? 'bg-gradient-to-b from-[#020c1a] to-[#041832] border border-primary/40 shadow-[0_24px_80px_rgba(2,132,199,0.4)] lg:scale-[1.04] lg:z-10'
                  : 'bg-white border border-slate-200 shadow-[0_10px_35px_rgba(15,23,42,0.06)] hover:shadow-[0_16px_50px_rgba(2,132,199,0.12)] hover:border-primary/30 hover:-translate-y-1 transition-all duration-300'
                }`}
              >
                {plan.highlighted && (
                  <>
                    <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-primary to-sky-400 rounded-t-3xl"></div>
                ...
                <div className={`${plan.highlighted ? 'text-5xl text-white' : 'text-4xl text-text-main'} font-headline font-bold mb-1 num-tabular`}>€{price}<span className={`text-sm font-normal ${plan.highlighted ? 'text-white/40' : 'text-text-muted'}`}>/mese</span></div>
                ...
                {isAnnual && <div className={`text-xs mb-1 ${plan.highlighted ? 'text-white/30' : 'text-text-muted'}`}>in un'unica soluzione</div>}
