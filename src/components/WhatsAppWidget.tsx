...
export default function WhatsAppWidget() {
  ...
  useEffect(() => {
    ...
    const n = window.setTimeout(() => setNudge(true), NUDGE_DELAY_MS);
    return () => {
  ...
  return (
    <div ref={ref} className={`wa-widget${visible ? " visible" : ""}${open ? " open" : ""}`}>
      ...
      {nudge && !open && (
        <div className="wa-nudge">
          <button type="button" className="wa-nudge-close" aria-label="Chiudi messaggio" onClick={() => setNudge(false)}>
            ×
          </button>
          <a
            ...
            rel="noopener noreferrer"
            onClick={() => {
              trackCta("whatsapp", "nudge_operatore");
              setNudge(false);
            }}
          >
